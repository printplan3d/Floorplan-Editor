...
/**
 * True for names the app generated rather than the user typing them.
 *
 * AutoRoomDetector names every zone it finds `Room <n>` (see
 * components/systems/zone/auto-room-detector.tsx), so a freshly detected
 * plan is full of "Room 9", "Room 11" and so on. Those are placeholders, not
 * something the user chose, and must not be mistaken for a custom name --
 * otherwise picking a room type leaves the label reading "Room 9" while the
 * type says Kitchen, which is exactly what a user reported.
 *
 * Deliberately anchored and digits-only: "Room 9" is generated, but
 * "Room for the kids" or "Games Room 2" are the user's and stay untouched.
 */
function isAutoGeneratedZoneName(name?: string): boolean {
  return /^room\s*\d+$/i.test((name ?? '').trim())
}
...
export function ZonePanel() {
  ...
  const isCustomName =
    (node.name || '').trim() !== '' &&
    node.name !== currentLabel &&
    !isAutoGeneratedZoneName(node.name)
  return (
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  // 2026-08-01: `Room 9` and friends are auto-detector
                  // placeholders and must rename too. The old check only
                  // matched the bare string 'Room', so every auto-detected
                  // room kept its number while its type silently changed --
                  // pick Kitchen, label still reads "Room 9".
                  const shouldRename =
                    currentName === '' ||
                    currentName === prevLabel ||
                    isAutoGeneratedZoneName(currentName)
                  handleUpdate({
                    roomType: rt.value,
